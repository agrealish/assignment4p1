...
<html>
  ...
  <body>
    <?php 
      ...
      function checkInt($_GET) { //is_int function came from PHP manual: http://php.net/manual/en/function.is-int.php
        if(!(is_int($_GET['min-multiplicand']))) { //if min-multiplicand isn't integer, print error message
          $eCheck=true; //an error occurred, so set eCheck to true
          echo "<p>min-multiplicand must be an integer.";
        }
        if(!(is_int($_GET['max-multiplicand']))) { //if max-multiplicand isn't integer, print error message
          $eCheck=true; //an error occurred, so set eCheck to true
          echo "<p>max-multiplicand must be an integer.";
        }
        if(!(is_int($_GET['min-multiplier']))) { //if min-multiplier isn't integer, print error message
          $eCheck=true; //an error occurred, so set eCheck to true
          echo "<p>min-multiplier must be an integer.";
        }
        if(!(is_int($_GET['max-multiplier']))) { //if max-multiplier isn't integer, print error message
          $eCheck=true; //an error occurred, so set eCheck to true
          echo "<p>max-multiplier must be an integer.";
        }
      }
      
      function checkMinMax($_GET) {
